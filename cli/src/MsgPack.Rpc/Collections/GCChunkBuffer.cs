...
namespace MsgPack.Collections
{
	...
	internal sealed class GCChunkBuffer : ChunkBuffer
	{
		...
		internal GCChunkBuffer( List<ArraySegment<byte>> chunks, int segmentSize, long initialTotalLength )
		{
			Contract.Assert( chunks != null );
			Contract.Assert( initialTotalLength >= 0 );
			Contract.Assert( segmentSize > 0 );
		...
		protected override void FillCore( IEnumerable<byte> values )
		{
			this._totalLength = 0;
			int segmentIndex = 0;
			int offset = 0;
			ArraySegment<byte> segment = this.GetFillingTarget( segmentIndex );
			foreach ( var b in values )
			{
				if ( offset == segment.Count )
				{
					segmentIndex++;
					segment = this.GetFillingTarget( segmentIndex );
					offset = 0;
				}

				segment.Set( offset, b );
				this._totalLength++;
			}
		}

		private ArraySegment<byte> GetFillingTarget( int index )
		{
			if ( index == this._chunks.Count )
			{
				var newSegment = new ArraySegment<byte>( new byte[ this._segmentSize ], 0, this._segmentSize );
				this._chunks.Add( newSegment );
				return newSegment;
			}
			else
		...
		protected sealed override ChunkBuffer ClipCore( long offset, long length )
		{
			...
			if ( offset == 0L && length == this.TotalLength )
			{
				...
				var clone = new GCChunkBuffer( new List<ArraySegment<byte>>( this._chunks ), this._segmentSize, this._totalLength );
				this._chunks.Clear();
				return clone;
			}
			...
			return new GCChunkBuffer( newChunks, this._segmentSize, newLength );
		}
